...
:::caution ts vs tsx
You can use **`js`** and **`.jsx`** files interchangeably with the right configuration. This is **not** the case with **`.ts`** and **`.tsx`** files at the moment.
You can only use JSX with TypeScript in a **`.tsx`** file.
:::
